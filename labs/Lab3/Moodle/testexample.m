%% set wave, medium and simulation parameters 
f = 1E8;    epsilon_r = 1;      mu_r = 4; 
...
arrow3([z zeros(size(z)) zeros(size(z)) ], [z Ex Ey ]) % plot field vectors using arrow3.m  
arrow3([z zeros(size(z)) zeros(size(z)) ], [z scaleH*Hx scaleH*Hy ]) 
legend('E field', 'H field'); grid on; xlabel('z'); ylabel('x'); zlabel('y')
% The following plot the 6 field components (Ex, Ey, Ez, Hx, Hy, Hz) for points on the z axis 
